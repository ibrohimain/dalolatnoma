      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-xl rounded-[2.5rem] border border-slate-200 overflow-hidden">
        <div className="bg-slate-900 p-8 text-white flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-2">Kitoblar ma'lumotlarini kiritish</p>
          </div>
        ...
        <div className="p-8 space-y-10">
          <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="flex flex-col gap-1 md:col-span-2">
              ...
              <input required type="text" name="actDescription" value={act.actDescription} onChange={handleActChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-slate-800 outline-none focus:border-blue-500" />
            </div>
          </section>
          ...
          <section className="bg-slate-50 p-6 rounded-[2rem] border border-slate-200 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Topshiruvchi F.I.SH</label>
          ...
          <section className="space-y-4">
            <div className="flex justify-between items-center border-b pb-4">
              ...
              <button type="button" onClick={addBook} className="bg-slate-900 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-slate-800 transition-all shadow-lg">+ Adabiyot qo'shish</button>
            </div>

            <div className="space-y-4">
              {act.books.map((book, idx) => (
                <div key={book.id} className="p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100 relative group hover:bg-white hover:shadow-xl transition-all">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    <div className="md:col-span-3">
                      ...
                      <input required type="text" name="author" value={book.author} onChange={(e) => handleBookChange(idx, e)} className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-xs outline-none focus:border-blue-500" placeholder="Muallif, Nomi, Turi" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="text-[8px] font-black text-slate-400 uppercase mb-1 block">ISBN</label>
                      <input required type="text" name="isbn" value={book.isbn} onChange={(e) => handleBookChange(idx, e)} className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-xs outline-none focus:border-blue-500" placeholder="ISBN raqami" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="text-[8px] font-black text-slate-400 uppercase mb-1 block">Bo'limi</label>
                      <input required type="text" name="department" value={book.department} onChange={(e) => handleBookChange(idx, e)} className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-xs outline-none focus:border-blue-500" placeholder="Bo'lim nomi" />
                    </div>
                    ...
                    <div className="md:col-span-1">
                      <label className="text-[8px] font-black text-slate-400 uppercase mb-1 block">Summasi</label>
                      <input required type="number" name="pricePerCopy" value={book.pricePerCopy} onChange={(e) => handleBookChange(idx, e)} className="w-full px-2 py-2 rounded-lg border border-slate-200 bg-white text-xs font-bold text-blue-600" />
                    </div>
                    <div className="md:col-span-1">
                      <label className="text-[8px] font-black text-slate-400 uppercase mb-1 block">Jami</label>
                      <div className="w-full px-2 py-2 rounded-lg bg-slate-100 border border-slate-200 text-xs font-black text-center text-slate-700">
